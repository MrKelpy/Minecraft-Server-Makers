...
def log(server_files_path, log_msg):
    ...
    with open(os.path.join(server_files_path, 'creator_logs.msf'), 'a+') as logs:
        now = datetime.now()
...
''')
        ...
        if ram <= 1024:
            print(f'WARNING - You have allocated {ram}MB of RAM to your server. That might not be enough. If you wish to change it,'
                  f'go to the ram_config.msf ({os.path.join(server_files_path, "ram_config.msf")}) file and adjust the value.')

    subprocess.run(
        ['java', f'-Xmx{ram+1}M', f'-Xms{ram}M', '-DIReallyKnowWhatIAmDoingISwear=true', '-jar', f'craftbukkit-{version}.jar', 'nogui'],
